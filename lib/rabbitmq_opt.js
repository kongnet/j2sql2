...
class RabbitMQ {
  ...
  async open() {
    ...
    this.conn.on('disconnect', function ({ err }) {
      console.error(
        $.c.r('✘'),
        `-x- RabbitMQ disconnect ${err.toString()} queueLength=${that.ch && that.ch.queueLength()}`)
    });
    this.conn.on('connectFailed', function ({ err }) {
      console.error(
        $.c.r('✘'),
        `-x- RabbitMQ connectFailed ${err.toString()}`
      )
    })
  ...
  async get(opt = {}) {
    ...
    try {
      ...
      const r = await ch.get(opt.queueName || this.opt.queueName, { noAck: noAck })
      return r
    } catch (e) {
