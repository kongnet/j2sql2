...
function extendDB (exObj = {}, db, tableName, option = {}) {
  ...
  if (
    ...
    $.err(
      $.c.r(
        `ex扩展失败 ${tableName} 表需要有删除标记：${option.delflagField} 创建时间：${option.createTimeField} 修改时间：${option.modifyTimeField}`
      )
    )
    return {
      errCode: 400,
      msg: `${tableName} 表需要有删除标记：${option.delflagField} 创建时间：${option.createTimeField} 修改时间：${option.modifyTimeField}`
    }
  }
