...
const DbOpt = function (db, tbName, exColumn) {
  let [me, sql, errObj, _name] = [this, '', { errCode: 0, msg: '' }, tbName]
  me.db = db
  me.errObj = null
  me.get = function () {
    // 返回生成的sql
  ...
  me.cmd = function (s, v = '') {
    ...
    sql += me.db.mysql.format(s, v) + _tail // 建议使用 preSql防止注入
    return me
  }
  ...
  me.where = function (o, type) {
    ...
    for (const i in o) {
      if (!field.includes(i.toLowerCase())) {
        return { errCode: 400, msg: `WHERE错误，表 ${_name} 没有 ${i} 列` }
      }
      const fieldType =
    ...
    */
    ...
    for (const i in b) {
      ...
      if (!field.includes(i.toLowerCase())) {
        return {
          run: function () {
            return { errCode: 400, msg: `列错误，表 ${_name} 没有 ${i} 列` }
          }
        }
      }
      cols.push('`' + i + '`')
